...
//    string read_extention(const std::string& filename) const;
    ...
    //       their page/filename
    std::map<std::string, WebPageSet> webPageSetMapWithTerm;
    std::map<std::string, WebPage*> webPageMapWithName;
    PageParser* noExtensionParser_;
    std::map<std::string,PageParser*> pageParserMap_;
    
// Enji: considering create a set for webPages, whenever encounter a link, check if that link is already existed
// If yes, add the current page to it's incoming link, else, created a new webpage, still add current page as the
// incoming link
//    double rankHelper(WebPage* start, double sigma, int times, WebPageSet& expandedSet, std::map<WebPage*, int>&);
    std::map<WebPage*,double> rankHelper(double epsilon, int times, WebPageSet& expandedSet, std::map<WebPage*, int>&);
